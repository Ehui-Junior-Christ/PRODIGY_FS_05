...
router.get("/me", requireAuth, async (req, res) => {
  try {
    const result = await client.execute({
      sql: `SELECT u.id, u.name, u.handle, u.email, u.avatar_url, u.cover_url, u.bio, u.created_at,
...
router.put("/me", requireAuth, async (req, res) => {
  try {
    const { name, handle, avatar_url, cover_url, bio } = req.body;
    ...
    await client.execute({
      sql: "UPDATE users SET name = ?, handle = ?, avatar_url = ?, cover_url = ?, bio = ? WHERE id = ?",
      args: [name, handle, avatar_url || null, cover_url || null, bio || null, req.user.id]
    });
    ...
    res.json({ message: "Profil mis à jour", token: newToken, user: { name, handle, avatar_url, cover_url, bio } });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: "Erreur serveur" });
  }
